...
const BOX_MIN_SIZE = 2; //min size in cm for image capture
  ...
  onAwake() {
    ...
    if (this.isEditor) {
      ...
      delayedEvent.bind(() => {
        ...
        this.captureRendMesh.mainPass.captureImage = ProceduralTextureProvider.createFromTexture(
  ...
  private processImage() {
    if (this.updateEvent != null) {
      ...
      this.chatGPT.makeImageRequest(this.captureRendMesh.mainPass.captureImage, (response) => {
        ...
        let hideEvent = this.createEvent("DelayedCallbackEvent");

        hideEvent.bind(() => {
            this.captureRendMesh.enabled = false;

            this.caption.scaleObj.enabled = false;
